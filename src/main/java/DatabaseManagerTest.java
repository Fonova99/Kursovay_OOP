...
public class DatabaseManagerTest {
    ...
    public void setUp() throws SQLException {
        databaseManager = Mockito.spy(new DatabaseManager());  // Создание объекта для тестирования
        mockConnection = mock(Connection.class);
        mockPreparedStatement = mock(PreparedStatement.class);
        mockTableModel = mock(DefaultTableModel.class);

        // Мокируем создание подключения
        doReturn(mockConnection).when(databaseManager).getConnection();
    ...
    public void testDeleteRowFromDoctors() throws SQLException {
        ...
        when(mockTableModel.getValueAt(rowIndex, 0)).thenReturn(doctorName);
